...
export const MergeConflictStudio: React.FC<MergeConflictStudioProps> = ({
  ...
  const handleRunCheck = async () => {
    ...
    if (res.conflicts && res.conflicts.length > 0) {
      ...
      res.conflicts.forEach((c: ConflictChunk) => {
  ...
  const currentConflict = mergeResult?.conflicts.find((c: ConflictChunk) => c.id === activeConflictId);
          ...
          <Card className="p-4 bg-card border-border flex flex-col gap-3">
            <div className="flex items-center justify-between border-b border-border pb-2">
              ...
              <Badge className="bg-rose-500/15 text-rose-400 text-[10px]">
                {mergeResult.conflicts.filter((c: ConflictChunk) => !c.isResolved).length} pending
              </Badge>
            </div>

            <div className="flex flex-col gap-1.5">
              {mergeResult.conflicts.map((c: ConflictChunk) => (
                <button
                  key={c.id}
                  type="button"
