...
screenRouter.get('/mcp-config', async (_req, res: Response) => {
  const { getOpenPencilMcpConfig } = await import('./mcpService.js');
  return res.json({
    success: true,
...
screenRouter.get('/mcp-status', async (_req, res: Response) => {
  const { checkOpenPencilMcpBridgeStatus } = await import('./mcpService.js');
